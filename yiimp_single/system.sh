...
case "$DISTRO" in
    "18")
        sudo add-apt-repository 'deb [arch=amd64,arm64,ppc64el] https://mirror.group.one/mariadb/mariadb-10.6.23/repo/ubuntu bionic main' >/dev/null 2>&1
        ;;
    "20")
        sudo add-apt-repository 'deb [arch=amd64,arm64,ppc64el,s390x] https://mirror.group.one/mariadb/mariadb-10.6.23/repo/ubuntu focal main' >/dev/null 2>&1
        ;;
    "16")
        sudo add-apt-repository 'deb [arch=amd64,arm64,i386,ppc64el] https://mirror.group.one/mariadb/mariadb-10.6.23/repo/ubuntu xenial main' >/dev/null 2>&1
        ;;
esac
echo -e "$GREEN Complete...$COL_RESET"
